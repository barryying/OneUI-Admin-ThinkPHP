 ...
 * 4，支持规则表达式。
 *      在admin_auth_rule 表中定义一条规则时，如果type为1， condition字段就可以定义规则表达式。 如定义{score}>5  and {score}<100
...
class Auth
{
     ...
     * session 前缀
     */
    const  SESSION_PREFIX       = '__admin__';
    ...
    private static function sessionGet($path =''){
        $user  = Session::get(self::SESSION_PREFIX . $path );
        return $user;
    }
}